...
exports.getRealTimeToken = async (req, res) => {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Approval API 요청 실패: ${response.status} ${errorBody}`
      );
    }
    ...
    res
      .status(500)
      .json({ success: false, message: "Error getRealTimeToken 오류" });
  }
};
...
exports.getDailyChart = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error in getDailyChart 오류" });
  }
};
...
exports.getMyFavoritesStocks = async (req, res) => {
  try {
    ...
    const data = await earningsService.getMyFavoritesStockFinances(
      myFavoritesStocks
    );
    ...
    res
      .status(500)
      .json({ success: false, message: "Error in getMyFavoritesStocks" });
  }
};
...
exports.getEarningsBySymbol = async (req, res) => {
  try {
    const stockId = await earningsRepository.getStockIdBySymbol(
      req.params.symbol
    );
    console.log(stockId);
